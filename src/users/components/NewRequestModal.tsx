...
export default function NewRequestModal({
  ...
  const isUpdateMode = submitLabel === "Update"
  ...
  const getDraftStorageKey = () => {
    try {
      ...
      const username = String(parsed?.username || "").trim()
      return `new_request_draft:${username || "_"}`
    } catch {
      return "new_request_draft:_"
    }
  }
  ...
  // When modal closes in update mode, forget the last applied payload so that
  // next open reloads fresh data from the server-sourced initialPayload.
  useEffect(() => {
    if (isOpen) return
  ...
  useEffect(() => {
    ...
    setActivePrPage(0)
    setSubmitError(null)
  }, [defaultFormData, defaultTableRows, initialPayload, isOpen, preparedByName])
  ...
  useEffect(() => {
    ...
    try {
      ...
      const draft = JSON.parse(raw) as {
        ...
        obrEnabled?: boolean
        activeTab?: "pr" | "obr"
        prPages?: Array<Array<{ itemNo?: string; unit?: string; description?: string; quantity?: string; unitCost?: string; totalCost?: string }>>
        activePrPage?: number
        deptHeadName?: string
        deptHeadDesignation?: string
        obrReps?: { leftName?: string; leftPosition?: string; rightName?: string; rightPosition?: string }
        prSignatories?: { requestedByName?: string; requestedByDesignation?: string; cashAvailabilityName?: string; cashAvailabilityDesignation?: string; approvedByName?: string; approvedByDesignation?: string }
      }
      ...
      if (typeof draft?.prEnabled === "boolean") setPrEnabled(draft.prEnabled)
      if (typeof draft?.obrEnabled === "boolean") setObrEnabled(draft.obrEnabled)
      if (draft?.activeTab === "pr" || draft?.activeTab === "obr") setActiveTab(draft.activeTab)

      if (Array.isArray(draft?.prPages) && draft.prPages.length > 0) {
        const nextPages = draft.prPages.map((page, pageIndex) => {
          const rows = Array.isArray(page) ? page : []
          const empty = createEmptyPrPageRows()
          const startIdx = pageIndex > 0 ? 1 : 0
          for (let i = startIdx; i < Math.min(empty.length, rows.length); i++) {
            const r = rows[i] || {}
            empty[i] = {
        ...
        setPrPages(nextPages)
        const nextActive = typeof draft.activePrPage === "number" ? draft.activePrPage : 0
        setActivePrPage(Math.max(0, Math.min(nextActive, nextPages.length - 1)))
      }

      if (typeof draft?.deptHeadName === "string") setDeptHeadName(draft.deptHeadName)
      if (typeof draft?.deptHeadDesignation === "string") setDeptHeadDesignation(draft.deptHeadDesignation)
      if (draft?.obrReps && typeof draft.obrReps === "object") {
        setObrReps((prev) => ({
          ...prev,
          leftName: typeof draft.obrReps?.leftName === "string" ? draft.obrReps.leftName : prev.leftName,
          leftPosition: typeof draft.obrReps?.leftPosition === "string" ? draft.obrReps.leftPosition : prev.leftPosition,
          rightName: typeof draft.obrReps?.rightName === "string" ? draft.obrReps.rightName : prev.rightName,
          rightPosition: typeof draft.obrReps?.rightPosition === "string" ? draft.obrReps.rightPosition : prev.rightPosition,
        }))
      }
      if (draft?.prSignatories && typeof draft.prSignatories === "object") {
        setPrSignatories((prev) => ({
          requestedByName: typeof draft.prSignatories?.requestedByName === "string" ? draft.prSignatories.requestedByName : prev.requestedByName,
          requestedByDesignation: typeof draft.prSignatories?.requestedByDesignation === "string" ? draft.prSignatories.requestedByDesignation : prev.requestedByDesignation,
          cashAvailabilityName: typeof draft.prSignatories?.cashAvailabilityName === "string" ? draft.prSignatories.cashAvailabilityName : prev.cashAvailabilityName,
          cashAvailabilityDesignation: typeof draft.prSignatories?.cashAvailabilityDesignation === "string" ? draft.prSignatories.cashAvailabilityDesignation : prev.cashAvailabilityDesignation,
          approvedByName: typeof draft.prSignatories?.approvedByName === "string" ? draft.prSignatories.approvedByName : prev.approvedByName,
          approvedByDesignation: typeof draft.prSignatories?.approvedByDesignation === "string" ? draft.prSignatories.approvedByDesignation : prev.approvedByDesignation,
        }))
      }
    } catch {
      setHasDraft(false)
  ...
  useEffect(() => {
    ...
    const payloadSig = (() => {
      try {
        return JSON.stringify(initialPayload)
      } catch {
        return ""
      }
    })()
    if (payloadSig && lastAppliedInitialPayloadRef.current === payloadSig) return
    lastAppliedInitialPayloadRef.current = payloadSig
    ...
    setFormData((prev) => ({
      ...
      responsibilityCenter:
        typeof initialPayload.responsibilityCenter === "string" ? initialPayload.responsibilityCenter : prev.responsibilityCenter,
    ...
    if (Array.isArray(initialPayload.prItems)) {
      ...
      const pagesItems: Array<typeof payloadItems> = (() => {
        const hasMarkers = payloadItems.some((it) => String(it?.description || "").trim() === PR_PAGE_BREAK_MARKER)
        if (!hasMarkers) return [payloadItems]

        const acc: Array<typeof payloadItems> = [[]]
        ...
        return acc.length ? acc : [payloadItems]
      })()

      const pagesNeeded = Math.max(1, pagesItems.length)
      const nextPages = Array.from({ length: pagesNeeded }).map(() => createEmptyPrPageRows())

      for (let pageIdx = 0; pageIdx < pagesItems.length; pageIdx++) {
        const pageItems = pagesItems[pageIdx]
        const startRowIdx = pageIdx > 0 ? 1 : 0
        const maxItemsThisPage = pageIdx > 0 ? 26 : 27
        for (let i = 0; i < Math.min(pageItems.length, maxItemsThisPage); i++) {
          const rowIdx = startRowIdx + i
          const it = pageItems[i]
          nextPages[pageIdx][rowIdx] = {
            itemNo: "",
            unit: String(it?.unit || ""),
    ...
    setPrSignatories((prev) => ({
      ...prev,
      requestedByName:
        typeof initialPayload.requestedByName === "string" && initialPayload.requestedByName.trim()
          ? initialPayload.requestedByName
          : prev.requestedByName,
      requestedByDesignation:
        typeof initialPayload.requestedByDesignation === "string" && initialPayload.requestedByDesignation.trim()
          ? initialPayload.requestedByDesignation
          : prev.requestedByDesignation,
      cashAvailabilityName:
        typeof initialPayload.cashAvailabilityName === "string" && initialPayload.cashAvailabilityName.trim()
          ? initialPayload.cashAvailabilityName
          : prev.cashAvailabilityName,
      cashAvailabilityDesignation:
        typeof initialPayload.cashAvailabilityDesignation === "string" && initialPayload.cashAvailabilityDesignation.trim()
          ? initialPayload.cashAvailabilityDesignation
          : prev.cashAvailabilityDesignation,
      approvedByName:
        typeof initialPayload.approvedByName === "string" && initialPayload.approvedByName.trim()
          ? initialPayload.approvedByName
          : prev.approvedByName,
      approvedByDesignation:
        typeof initialPayload.approvedByDesignation === "string" && initialPayload.approvedByDesignation.trim()
          ? initialPayload.approvedByDesignation
          : prev.approvedByDesignation,
    }))

    setObrReps((prev) => ({
      ...prev,
      leftName:
        typeof initialPayload.certifiedAName === "string" && initialPayload.certifiedAName.trim()
          ? initialPayload.certifiedAName
          : prev.leftName,
      leftPosition:
        typeof initialPayload.certifiedAPosition === "string" && initialPayload.certifiedAPosition.trim()
          ? initialPayload.certifiedAPosition
          : prev.leftPosition,
      rightName:
        typeof initialPayload.certifiedBName === "string" && initialPayload.certifiedBName.trim()
          ? initialPayload.certifiedBName
          : prev.rightName,
      rightPosition:
        typeof initialPayload.certifiedBPosition === "string" && initialPayload.certifiedBPosition.trim()
          ? initialPayload.certifiedBPosition
          : prev.rightPosition,
    }))
                  ...
                  <div className="border border-black relative min-h-[600px] p-4 max-w-2xl mx-auto" style={{ fontFamily: 'Times New Roman, Times, serif' }}>
                    ...
                    <div className="mt-2 border border-black min-h-[500px] p-0 mb-1">
                      ...
                      {tableRows.map((row, index) => {
                        ...
                        const useGlobalAuto = isUpdateMode && storedItemNo && localAutoItemNo && storedItemNo === localAutoItemNo
